...
var UpdateLong = `Print the build version for c8s`
...
Updates available. v%s -> %s
  ...
  run 'c8s update' to get the latest.
...
func CheckUpdate(manual bool) (ver ProgramVersion, err error) {
	if !manual && os.Getenv("C8S_UPDATES_DISABLED") != "" {
		return
	}
	UpdateStarted = true
	cur := ProgramVersion{Version: "v" + verinfo.Version}

	checkURL := "https://api.github.com/repos/hofstadter-io/c8s/releases/latest"
	if UpdateVersionFlag != "" {
		checkURL = "https://api.github.com/repos/hofstadter-io/c8s/releases/tags/" + UpdateVersionFlag
		manual = true
	}
...
func WaitPrintUpdateAvailable() {
	if os.Getenv("C8S_UPDATES_DISABLED") != "" {
		return
	}
